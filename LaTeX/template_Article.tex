...
\author{Isabelle Joosten\\Student number 4998480\\Assignment 1 for the course ``Physics of Planetary Interiors" (AE4893-24)\\Total time spent: 70 hours}
...
\begin{table*}[h]
	...
	\centering
		\begin{tabular}{|>{\raggedright\arraybackslash}m{0.45\columnwidth}|>{\raggedright\arraybackslash}m{0.2\columnwidth}|>{\raggedright\arraybackslash}m{0.37\columnwidth}|>{\raggedright\arraybackslash}m{0.43\columnwidth}|>{\raggedright\arraybackslash}m{0.25\columnwidth}|}
			\hline
			& \textbf{Jin and Ji} & \textbf{Cammarano et al.} & \textbf{Kuskov and Kronrod} & \textbf{Howell}\\ \hline
			\textbf{Total radius {[}km{]}} & 1563 & Not provided & 1565 & 1561\\ \hline
			...
			\textbf{Centre pressure {[}GPa{]}} & $\sim$5.5 & Not provided & $\sim$4.8 & Not provided \\ \hline
		\end{tabular}%
\end{table*}
...
\end{equation}
This is then compared to the observed dimensionless moment of inertia. If the calculated moment of inertia is higher than the observed value, the shell thickness is increased, and if it is lower, the shell thickness is decreased. The total calculated mass is also compared with the observed mass, and if the calculated mass is higher than the observed value, the core radius is decreased, and vice versa. Lastly, the mean density is calculated and compared with the observed mean density, based on which both the core and mantle boundaries are adjusted. The simulation is run again with the adjusted values, and it continues in this loop until both the calculated mass moment of inertia, and mean density are within the uncertainty range of the observed value. The observed values can be found in Table \ref{tab:observe}.\\
...
\begin{itemize}
	...
	\item Europa's observed radius is 1560.80 $\pm$ 0.3 km \cite{};
	\item Europa's observed mass is ()479.7 $\pm$ 1.5)*10\textsuperscript{20} kg \cite{};
	\item Europa's observed dimensionless moment of inertia is 0.346 $\pm$ 0.005 \cite{}.
\end{itemize}
...
Running the program with these inputs results in the values shown in Table \ref{}. Furthermore, the model predicts the pressure at the centre of Europa to be 5.1 GPa, which roughly agrees with the values in Table \ref{tab:modelcomparison}. The gravitational acceleration at the surface is found to be 1.32 m/s\textsuperscript{2}, which matches the value 1.31 m/s\textsuperscript{2} given by Cammarano et al.\cite{cammaranoLongperiodSeismologyEuropa2006}. The core radius and shell thickness found are 683 and 116 km, respectively, most closely resembling the models by Cammarano et al.\cite{cammaranoLongperiodSeismologyEuropa2006} and Howell\cite{howellLikelyThicknessEuropas2021}. It takes the program 137 iterations to converge to these values.\\
