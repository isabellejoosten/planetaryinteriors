...
\author{Isabelle Joosten\\Student number 4998480\\Assignment 1 for the course ``Physics of Planetary Interiors" (AE4893-24)\\Total time spent: 60 hours}
...
\begin{itemize}
	...
	\item Europa's observed radius is  1560.80 $\pm$ 0.30 km \footnote{Information taken from NASA's Solar System Dynamics page (https://ssd.jpl.nasa.gov/sats/phys\_par/), accessed on 13/01/2026.};
	\item Europa's observed mass is (479.7 $\pm$ 1.5)*10\textsuperscript{20} kg \cite{yoderAstrometricGeodeticProperties2013};
	\item Europa's observed dimensionless moment of inertia is 0.346 $\pm$ 0.005 \cite{schubertInteriorCompositionStructure2004}.
\end{itemize}
...
\begin{table}[]
	...
	\resizebox{\columnwidth}{!}{%
		\begin{tabular}{|l|l|l|l|}
			...
			\textbf{Gravity at surface [m/$s^2$]} & 1.294   & 1.295   & 1.295   \\ \hline
...
As mentioned before in Section \ref{sec:lit}, some models found in literature do assume a core made of pure iron, so this is an avenue worth exploring. As seen in Section \ref{sec:model3}, the moment of inertia factors found by model III closely match the observed value, but the mass found is significantly larger than the observed one. This implies one of two things: Europa's core is less dense than one made of pure iron, or Europa's core is smaller than it is assumed to be in model III. Running an iterative version of model III (in much the same way as was done for model II) leads to the program decreasing the core radius to 0 without actually converging to the observed mass, while the moment of inertia factor converges to the observed value immediately. One could conclude from this that Europa's core cannot be made of pure iron as it is too dense to fit both the observed mass and moment of inertia factor. However, it should be noted that there are some flaws in this iterative model. Most glaringly, the pressure and gravity values at the layer boundaries are not iterated. Further research is needed to find out of this conclusion is actually valid or not.
...
\section{Conclusion}
In this report, four interior models of Europa from literature were analysed and compared to each other. Furthermore, three new models were constructed and compared to both each other and the models found in literature. The first two models have shown to provide a decent first estimate for the interior profile of Europa, and they roughly agree with those found in literature. However, there is a lot of uncertainty in the input values taken from literature. Future missions to Europa could provide more accurate data that can be used to construct more accurate models. The third model agrees with the observed moment of inertia factor, but not with the observed mass. This model could in the future be modified to contain a more accurate layer distribution. The models are quite simplified, and neglect many factors necessary to create a realistic model. In future research, these models could perhaps be expanded to include more than 3 layers, and thermal effects such as tidal heating or radiogenic heating.
